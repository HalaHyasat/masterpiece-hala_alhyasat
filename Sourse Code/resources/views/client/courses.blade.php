                                ...
                                @if($course->id != 0)
                                    <li>
                                        <a href="{{ route('courses.show', $course->id)}}">
                                            <img src="{{asset('assets/images/pages/'.$course->image)}}" class="w-full h-48 rounded-lg shadow-sm object-cover">
                                            <div class="pt-2">
                                                <h4 class="text-lg font-semibold"> {{$course->name}}  </h4>
                                                <p class="text-sm" style="text-overflow: ellipsis;  width: 10em; white-space: nowrap; overflow: hidden;">{{$course->desc}} </p>
                                            </div>
                                        </a>
                                    </li>
                                @endif
                                ...
                                @if ($limit < 7)
                                    <div class="flex items-center space-x-4 rounded-md -mx-2 p-2">
                                        <div class="w-14 h-14 flex-shrink-0 rounded-md relative">
                                            <img src="{{asset('assets/images/pages/'.$course->image)}}" class="absolute w-full h-full inset-0 rounded-md" alt="">
                                        </div>
                                        <div class="flex-1">
                                            <a href="{{ route('courses.show', $course->id)}}" class="text-lg font-semibold capitalize"> {{$course->name}} </a>
                                            <div class="text-sm text-gray-500 mt-0.5" style="text-overflow: ellipsis;  width: 20em; white-space: nowrap; overflow: hidden;"> {{$course->desc}}</div>
